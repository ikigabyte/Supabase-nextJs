...
export function ViewersDropdown({
  ...
  const getAvatarInfo = (user_id: string) => {
    ...
    const secondLetter =
      trimmed && trimmed.length > 1 ? trimmed[1].toUpperCase() : "";
    ...
    return { firstLetterCombined, color };
  };
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <div className="flex items-center space-x-1">
        {combined.map((v) => {
          const { firstLetterCombined, color } = getAvatarInfo(v.user_id);
          return (
            <div
              key={v.user_id}
              ...
              style={{
                ...
                opacity: v.idle ? 0.4 : 1, // fade idle viewers
              }}
              data-testid={`viewer-${v.user_id}`}
            >
              {firstLetterCombined}
            </div>
          );
